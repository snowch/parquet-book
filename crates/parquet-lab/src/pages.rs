...
/// Read one page: its header, and the body the header measures. The reader is left after it.
///
/// A reader that knows where a page starts, from the page index (ch09), reads just that page
/// this way, without walking the pages before it (ch10).
pub fn read_page(r: &mut ByteReader) -> Result<Page, MetadataError> {
    let header = read_struct(r)?;
