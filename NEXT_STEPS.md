...
## Now: editing and running the code online

1. An editor in the page for the Python reader and the problems: edit a module or a stub, run the
   lab on the edited code, and run a problem's tests, all under Pyodide.
2. A Codespaces configuration for the Rust reader, which cannot compile in a page: the toolchain,
   the WebAssembly target and the Python packages, ready to run `make check`.
