...
The reader exists in Python as well as Rust (PLAN.md §4). Done: the foundation and ch01 to ch03.
...
1. ch04, ch05: `rle`, `plain`, `column`, `nested`, `decode`, `delta`, `encoding`.
2. ch06, ch07: page values, and the three decompressors.
3. ch08 to ch14: `stats`, `page_index`, `bloom` and `prune`; `scan`; `engine`; `crypto`; `table`.
