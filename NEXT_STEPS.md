...
The reader exists in Python as well as Rust (PLAN.md §4). Done: the foundation and ch01, ch02.
...
1. ch03: `logical` in full, `schema`, and the `schema` report.
2. ch04, ch05: `rle`, `plain`, `column`, `nested`, `decode`, `delta`, `encoding`.
3. ch06, ch07: page values, and the three decompressors.
4. ch08 to ch14: `stats`, `page_index`, `bloom` and `prune`; `scan`; `engine`; `crypto`; `table`.
